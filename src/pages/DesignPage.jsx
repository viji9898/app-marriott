...
const designPriorities = [
  ...
  ["Support efficient operation", "Guest, staff, goods and service circulation were planned separately."],
  ["Accommodate a full resort programme", "Rooms, dining, recreation, banqueting and back-of-house functions were integrated."],
  ["Remain adaptable", "Structural decisions avoided unnecessary long-term constraints."],
];
...
const flowGroups = [
  ...
  ["Goods and services", "Loading, storage, preparation, distribution and waste"],
];
...
const roomStandards = [
  ["The view", "Floor-to-ceiling glazing and the balcony maintained the visual connection with the bay."],
  ["The sense of space", "Sliding partitions allowed the bathroom and sleeping areas to become one continuous room or separate for privacy."],
  ["The operating standard", "The mock-up established the benchmark for joinery, sanitaryware, lighting, technology and finishes."],
];
...
const venues = [
  ["Weligama Kitchen", "High-capacity all-day dining with indoor and outdoor seating."],
  ["Big Fish", "A more intimate seafood restaurant with a darker maritime material palette."],
  ["Tides", "A lobby-level bar divided into active and quieter seating zones."],
  ["Pool and beach service", "Informal food and beverage provision connected to outdoor recreation."],
];
...
const designLessons = [
  ["Appoint the operator before fixing the design", "Late operator involvement required completed work to be reconsidered and contributed to delay."],
  ["Complete the area programme first", "Room counts, public areas, kitchens, servicing and back-of-house requirements should be resolved before design advances."],
  ["Design from operations outward", "Guest experience depends on service routes, preparation areas, storage and engineering systems that remain largely unseen."],
  ["Use full-scale mock-ups", "A completed reference room is more effective than drawings alone in controlling quality across repeated units."],
  ["Protect the defining proposition", "The sea-facing room configuration and arrival view remained intact through changes in scale, brand and specification."],
  ["Evaluate cost through value", "Higher material expenditure is justified only where it improves durability, operation, positioning or revenue potential."],
];
  ...
  return (
    <main className="design-page">
        ...
        <div className="page-width design-hero-content">
          ...
          <div className="design-hero-copy">
            <p>
              ...
              final room programme: every guest room would face the Indian Ocean.
            </p>
            <p>
      ...
      <section className="design-section page-width">
        <DesignHeading number="01" label="THE DESIGN BRIEF" title="Design had to support both experience and operation." />
      ...
      <section className="design-site">
        <div className="page-width design-site-grid">
          <DesignHeading number="02" label="RESPONDING TO THE SITE" title="Limited land required a vertical solution." />
          <div>
            <p>
            ...
            <p className="design-emphasis">The design therefore moved upwards.</p>
            <p>
              ...
              concept, but supported the project’s operating and investment objectives.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="design-ocean page-width">
        <DesignHeading number="03" label="EVERY ROOM FACING THE OCEAN" title="The least efficient option supported the strongest proposition." />
        ...
        <div className="design-corridor-comparison" aria-label="Diagrammatic corridor comparison, not to scale">
          <article>
      ...
      <section className="design-operator">
        <div className="page-width design-operator-grid">
          <div>
            <DesignHeading number="04" label="INTEGRATING THE OPERATOR" title="Marriott’s appointment materially changed the brief." />
            <p>
              Early architectural work began before an operator had been secured.
              Initial discussions with Marriott were based on a Courtyard
              positioning before the project moved to the full Marriott Resort brand.
            </p>
            <p>
              The higher finish and more comprehensive programme introduced the
              operating discipline required for an internationally managed resort.
              Some work had already commenced, so revised standards also produced
              redesign, remobilisation, delay and additional cost.
            </p>
          </div>
      ...
      <section className="design-flow page-width">
        <DesignHeading number="05" label="DESIGNING FOR FLOW" title="The hotel was planned as an operating system." />
        ...
        <div className="design-flow-groups">
          {flowGroups.map(([group, detail], index) => (
            <article key={group}>
              <span>0{index + 1}</span><h3>{group}</h3><p>{detail}</p>
              {index < flowGroups.length - 1 && <MoveRight size={18} />}
            </article>
          ))}
        </div>
        <div className="design-implications">
          {flowImplications.map((item) => <span key={item}>{item}</span>)}
        </div>
      ...
      <section className="design-structure">
        <div className="page-width design-structure-grid">
          <DesignHeading number="06" label="THE STRUCTURAL RESPONSE" title="Open spaces required changes below the finishes." />
          <div>
            <p>
              A post-tensioned structural solution reduced internal columns and
              beams across the ballroom and major public areas, where flexibility
              and clear sightlines were important.
            </p>
            <div className="design-outcomes">
              {structuralOutcomes.map((item) => <span key={item}>{item}</span>)}
            </div>
            <p>
              A dedicated service floor accommodated pipework, ducting,
              chilled-water systems and central controls. Major engineering
              functions remained separate from guest rooms and accessible for maintenance.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="design-rooms page-width">
        <DesignHeading number="07" label="THE GUEST ROOMS" title="One room became the control standard." />
      ...
      <section className="design-interiors">
        <div className="page-width design-interiors-grid">
          <div>
            <DesignHeading number="08" label="INTERIOR DESIGN" title="Local character was expressed through material and detail." />
            <p>
              Wilson Associates served as a key interior-design consultant within
              Marriott’s review and approval process. The project team coordinated
              room concepts, material selections and individual public spaces.
            </p>
            <p>
              Local references were restrained rather than literal or heavily themed.
            </p>
          </div>
          <div className="design-material-list">
            {interiorDetails.map((item, index) => <div key={item}><span>0{index + 1}</span><p>{item}</p></div>)}
          </div>
        </div>
      </section>

      <section className="design-arrival page-width">
        <DesignHeading number="09" label="THE ARRIVAL EXPERIENCE" title="The lobby framed the principal asset." />
        <div className="design-arrival-grid">
          <div>
            <p>
              The triple-height lobby used full-height columns to frame the ocean
              view. Muted internal lighting increased contrast with the exterior,
              while a dark stone floor reflected the horizon.
            </p>
            <p className="design-emphasis">
          ...
          <div className="design-lobby-functions">
            {lobbyFunctions.map((item) => <span key={item}>{item}</span>)}
          </div>
        </div>
      </section>
      ...
      <section className="design-venues">
        <div className="page-width">
          <DesignHeading number="10" label="FOOD AND BEVERAGE SPACES" title="Each venue was given a distinct operating role." />
          <div className="design-venue-table">
            {venues.map(([venue, role], index) => (
              <article key={venue}><span>{String(index + 1).padStart(2, "0")}</span><h3>{venue}</h3><p>{role}</p></article>
            ))}
          </div>
          <p className="design-venue-note">
            Venue planning considered circulation, service speed, dwell time and
            revenue alongside visual identity. Weligama Kitchen used clear buffet
            and live-cooking zones; Big Fish used a more enclosed material palette
            while retaining the ocean relationship.
          </p>
        </div>
      </section>

      <section className="design-landscape page-width">
        <DesignHeading number="11" label="LANDSCAPE AND EXTERNAL AREAS" title="The external programme completed the resort." />
        <div className="design-landscape-grid">
          <div>
            <p>
              Belt Collins served as a key landscape consultant. Pool, terraces,
              planting and beachfront areas formed part of the core guest programme
              rather than residual space around the building.
            </p>
            <p>
              Mature trees and plants were selected in advance for relocation as
              construction approached completion, allowing the landscape to appear
              established when the hotel opened.
            </p>
          </div>
          <div className="design-landscape-list">
            {landscapeElements.map((item) => <span key={item}>{item}</span>)}
          </div>
        </div>
      </section>
      ...
      <section className="design-value">
        <div className="page-width">
          <DesignHeading number="12" label="VALUE ENGINEERING" title="Cost discipline was applied without abandoning the brief." />
          <div className="design-value-intro">
            <p>
              ...
              expensive specifications were sourced and submitted to Marriott for approval.
            </p>
            <p>
              Patterned timber screens became the principal example. When specialist
              quotations proved uneconomic, the project acquired CNC equipment and
              manufactured the screens directly. Completed components were approved by Marriott.
            </p>
          </div>
          <div className="design-value-process">
            {valueProcess.map((step, index) => (
              <div key={step}><span>0{index + 1}</span><strong>{step}</strong>{index < valueProcess.length - 1 && <ArrowRight size={16} />}</div>
            ))}
          </div>
          <p className="design-safety-note">Fire, life-safety and essential operating standards were not treated as value-engineering opportunities.</p>
        </div>
      </section>

      <section className="design-lessons page-width">
        <DesignHeading number="13" label="DESIGN LESSONS" title="What the process established." />
        ...
        <div className="page-width design-close-grid">
          ...
          <div>
            ...
            <p>
              ...
              completed asset while managing approvals, procurement, contractors,
              financing and construction risk.
            </p>
            <Link className="design-next" to="/construction">
              <span>Next chapter</span><strong>Delivery and Construction</strong><ArrowRight size={20} />
            </Link>
          </div>
        </div>
      </section>
